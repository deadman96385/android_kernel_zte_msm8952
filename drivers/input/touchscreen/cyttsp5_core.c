...
extern struct cyttsp5_core_commands *cmd;
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_PLATFORM_FW_UPGRADE
extern struct cyttsp5_touch_firmware *cyttsp5_get_platform_firmware(
		struct device *dev);
extern int upgrade_firmware_from_platform(struct device *dev,
		bool forced);
extern int cyttsp5_check_firmware_version(struct device *dev,
		u32 fw_ver_new, u32 fw_revctrl_new);

#endif
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_PLATFORM_TTCONFIG_UPGRADE
extern struct cyttsp5_touch_config *cyttsp5_get_platform_ttconfig(
		struct device *dev);
extern int upgrade_ttconfig_from_platform(struct device *dev);
extern int cyttsp5_get_ttconfig_version(struct device *dev,
		const u8 *ttconfig_data, int ttconfig_size, u16 *version);
extern int cyttsp5_check_ttconfig_version(struct device *dev,
...
char *cyttsp_fwfile_table[CY_MOUDLE_NUM_MAX]=
{
CY_TPK_FW_NAME,
CY_TURLY_FW_NAME,
CY_SUCCESS_FW_NAME,
		...
		struct cyttsp5_core_data *cd, u8 *panel_id)
{
	...
	if (rc == -EPROTO && cd->response_buf[5] == ERROR_COMMAND) {
		printk("%s: Get Panel ID command not supported\n",
			__func__);
...
#define VREG_VBUS	"vcc_i2c"
 static int cyttsp_pinctrl_init(struct cyttsp5_core_data *cd)
{
	int retval;
	...
	if (IS_ERR_OR_NULL(cd->cpdata->ts_pinctrl)) {
		...
		retval = PTR_ERR(cd->cpdata->ts_pinctrl);
		cd->cpdata->ts_pinctrl = NULL;
		return retval;
	}

	cd->cpdata->gpio_state_active
		= pinctrl_lookup_state(cd->cpdata->ts_pinctrl, "pmx_ts_active");
	if (IS_ERR_OR_NULL(cd->cpdata->gpio_state_active)) {
		dev_dbg(cd->dev,
			"Can not get ts default pinstate\n");
		retval = PTR_ERR(cd->cpdata->gpio_state_active);
		cd->cpdata->ts_pinctrl = NULL;
		return retval;
	}

	cd->cpdata->gpio_state_suspend
		= pinctrl_lookup_state(cd->cpdata->ts_pinctrl, "pmx_ts_suspend");
	if (IS_ERR_OR_NULL(cd->cpdata->gpio_state_suspend)) {
		dev_dbg(cd->dev,
			"Can not get ts sleep pinstate\n");
		retval = PTR_ERR(cd->cpdata->gpio_state_suspend);
		cd->cpdata->ts_pinctrl = NULL;
		return retval;
	}
...
static int cyttsp_pinctrl_select(struct cyttsp5_core_data *cd,
						bool on)
{
	struct pinctrl_state *pins_state;
	int ret;

	pins_state = on ? cd->cpdata->gpio_state_active
		: cd->cpdata->gpio_state_suspend;
	if (!IS_ERR_OR_NULL(pins_state)) {
		ret = pinctrl_select_state(cd->cpdata->ts_pinctrl, pins_state);
		if (ret) {
			dev_err(cd->dev,
				"can not set %s pins\n",
...
static struct cyttsp5_core_data *cyttsp5_cd;
static int cy_update_flag=0;
...

{
	...
	cydata = &cyttsp5_cd->sysinfo.cydata;

	len += sprintf(page + len, "Manufacturer : %s\n", "cyttsp");
	len += sprintf(page + len, "panel id : 0x%x\n",cyttsp5_cd->panel_id);
	len += sprintf(page + len, "Firmware Major Version : 0x%x\n", cydata->fw_ver_major);
	len += sprintf(page + len, "Firmware Minor Version : 0x%x\n", cydata->fw_ver_minor);
	len += sprintf(page + len, "Revision Control Numbe : 0x%x\n", cydata->revctrl);
	len += sprintf(page + len, "Firmware Configuration Version : 0x%x\n", cydata->fw_ver_conf);
/*
"Bootloader Major Version", cydata->bl_ver_major,
"Bootloader Minor Version", cydata->bl_ver_minor,
"Protocol Major Version", cydata->pip_ver_major,
...
*/
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_PLATFORM_FW_UPGRADE
	ld=cyttsp5_cd->cyttsp5_dynamic_data[CY_MODULE_LOADER];
	ld->si = cmd->request_sysinfo(cyttsp5_cd->dev);

	fw = cyttsp5_get_platform_firmware(cyttsp5_cd->dev);
	if (!fw || !fw->img || !fw->size) {
		len += sprintf(page + len, "no platform fw to update \n");
	}else{

        if (!fw->ver || !fw->vsize) {
			len += sprintf(page + len, "platform fw Version : unknow\n");
        }else{
			fw_ver_new = get_unaligned_be16(fw->ver + 2);
			/* 4 middle bytes are not used */
			fw_revctrl_new = get_unaligned_be32(fw->ver + 8);
			upgrade = cyttsp5_check_firmware_version(cyttsp5_cd->dev, fw_ver_new,
				fw_revctrl_new);
			len += sprintf(page + len, "platform fw Version : 0x%x\n",fw_ver_new);
			len += sprintf(page + len, "platform fw Revision Control Numbe : 0x%x\n",fw_revctrl_new);
			len += sprintf(page + len, "update flag : 0x%x\n", cy_update_flag);
			len += sprintf(page + len, "fw need update : %s\n", (upgrade?"yes":"no"));
        }
	}
#endif
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_PLATFORM_TTCONFIG_UPGRADE

	ttconfig = cyttsp5_get_platform_ttconfig(cyttsp5_cd->dev);
	if (!ttconfig) {
         	...
         	;//dev_info(dev, "%s: No touch parameters\n", __func__);
              ...
              if (!param_regs->data || !param_regs->size) {
                  ...
                  if (!ttconfig->fw_ver || !ttconfig->fw_vsize) {
					  ...
					  upgrade=cyttsp5_check_ttconfig_version(cyttsp5_cd->dev, ttconfig->param_regs->data,
								  ttconfig->param_regs->size);
					  cyttsp5_get_ttconfig_version(cyttsp5_cd->dev, ttconfig->param_regs->data,
				ttconfig->param_regs->size, &cfg_ver_new);
					  len += sprintf(page + len, "platform config Version : 0x%x\n",cfg_ver_new);
					  len += sprintf(page + len, "config need update : %s\n", (upgrade?"yes":"no"));
				  }
              }
         }
	}
...

{
	...
	cy_update_flag = 0;
	printk("cyttsp  fw update begin\n");
	
	ld=cyttsp5_cd->cyttsp5_dynamic_data[CY_MODULE_LOADER];
	ld->si = cmd->request_sysinfo(cyttsp5_cd->dev);
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_PLATFORM_FW_UPGRADE
		if (!upgrade_firmware_from_platform(cyttsp5_cd->dev, false))
			printk("cyttsp update platform fw failed!\n");
#endif
	
	
#ifdef CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_PLATFORM_TTCONFIG_UPGRADE
		if (!upgrade_ttconfig_from_platform(cyttsp5_cd->dev))
			printk("cyttsp update platform ttconfig failed!\n");
#endif
printk("cyttsp fw update finsh\n");

cy_update_flag = 2;

#if 0
#if defined(CONFIG_TOUCHSCREEN_CYPRESS_CYTTSP5_LOADER)
	cy_update_flag = 0;
	if(touch_moudle>=CY_MOUDLE_NUM_MAX)
	{
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	printk("cyttsp set panel id to not enabled");
	dev_set_drvdata(dev, cd);
	cyttsp5_add_core(dev);
	
	rc = cyttsp_pinctrl_init(cd);
	if (!rc && cd->cpdata->ts_pinctrl) {
		rc = cyttsp_pinctrl_select(cd, true);
		if (rc < 0)
			//goto err_query_device;
			printk("cyttsp_pinctrl_select failed!\n");
	}

	pdata->core_pdata->vdd				= VREG_VDD;
	pdata->core_pdata->vbus				= VREG_VBUS;

	rc=touchscreen_gpio_init(cd->dev,1,pdata->core_pdata->vdd,pdata->core_pdata->vbus,
	pdata->core_pdata->rst_gpio,pdata->core_pdata->irq_gpio);
	if ( rc < 0 ){
		pr_err("%s, gpio init failed! %d\n", __func__, rc);
		//goto err_regulator;
	}
	//touchscreen_reset(0,pdata->core_pdata->rst_gpio);
	//touchscreen_power(0);
	//msleep(100);
	touchscreen_power(1);
	//msleep(10);
	//touchscreen_reset(1,pdata->core_pdata->rst_gpio);
	//msleep(200);
