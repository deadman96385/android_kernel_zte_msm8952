...
static int enable_escore_clk(int enable)
{
	...
	if (enable) {
		...
		if (clk_users == 1)
			goto exit;
pr_err("%s: liyunteng00000000000000000\n", __func__);
		if (escore_clk) {
			pr_err("%s: liyunteng1111111111111111111\n", __func__);
			clk_prepare_enable(escore_clk);
			clk_set_rate(escore_clk, 19200000);
			clk_users++;
		} else {
...
//weizj add for audience es804
	...
	dev_info(dev, "%s dev_name (%s)\n", __func__, dev_name(dev));
       pr_err("%s: liyunteng test bbclk\n", __func__);
	mutex_init(&cdc_mclk_mutex);
	escore_clk = clk_get(dev, "bb_clk2");
	enable_escore_clk(1);
	clkrate = clk_get_rate(escore_clk);
	dev_info(dev, "%s, clk rate %lu\n", __func__, clkrate);
