 ...
 */
int cyttsp5_check_firmware_version(struct device *dev,
		u32 fw_ver_new, u32 fw_revctrl_new)
{
	...
	printk("%s: img vers:0x%04X new vers:0x%04X\n", __func__,
	...
	printk("%s: img revctrl:0x%04X new revctrl:0x%04X\n",
...
struct cyttsp5_touch_firmware *cyttsp5_get_platform_firmware(
		struct device *dev)
{
	struct cyttsp5_loader_data *ld = cyttsp5_get_loader_data(dev);
...
int upgrade_firmware_from_platform(struct device *dev,
		bool forced)
{
	struct cyttsp5_loader_data *ld = cyttsp5_get_loader_data(dev);
	struct cyttsp5_touch_firmware *fw;
...
int cyttsp5_get_ttconfig_version(struct device *dev,
...
int cyttsp5_check_ttconfig_version(struct device *dev,
		const u8 *ttconfig_data, int ttconfig_size)
{
	struct cyttsp5_loader_data *ld = cyttsp5_get_loader_data(dev);
	u16 cfg_crc_new;
	...
	if (ld->loader_pdata->flags &
		...
		printk("%s: img_ver:0x%04X new_ver:0x%04X\n",
	...
	/* Check for config CRC */
		...
		printk("%s: img_crc:0x%04X new_crc:0x%04X\n",
...
struct cyttsp5_touch_config *cyttsp5_get_platform_ttconfig(
		struct device *dev)
{
	struct cyttsp5_loader_data *ld = cyttsp5_get_loader_data(dev);
...
int upgrade_ttconfig_from_platform(struct device *dev)
{
	struct cyttsp5_loader_data *ld = cyttsp5_get_loader_data(dev);
	struct cyttsp5_touch_config *ttconfig;
