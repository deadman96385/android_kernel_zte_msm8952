	...
	int btn_no, int btn_state)
{
	...
	switch(btn_no){
		case 0:
			...
			break;
		case 1:
			x=240*720/480+offset;
			break;
		case 2:
			x=400*720/480+offset;
			break;
		default:
			break;
		}

		y=920*1080/854+offset;
	//input_report_key(bd->input, si->btn[btn_no].key_code, btn_state);
	input_mt_slot(bd->input, 0);
	input_mt_report_slot_state(bd->input,MT_TOOL_FINGER, btn_state != 0);

	input_report_key(bd->input,BTN_TOUCH, 1);
	input_report_key(bd->input,BTN_TOOL_FINGER, 1);
	input_report_abs(bd->input,ABS_MT_POSITION_X,x);
	input_report_abs(bd->input,ABS_MT_POSITION_Y,y);
	input_report_abs(bd->input,ABS_MT_PRESSURE,btn_state);
...
static int cyttsp5_setup_input_device(struct device *dev)
{
	...
	__set_bit(EV_ABS, bd->input->evbit);
	...
	__set_bit(INPUT_PROP_DIRECT, bd->input->propbit);
    input_mt_init_slots(bd->input, 16,0);     // in case of "out of memory"
    set_bit(BTN_TOUCH, bd->input->keybit);
	set_bit(BTN_TOOL_FINGER, bd->input->keybit);
    input_set_abs_params(bd->input, ABS_MT_POSITION_X, 0, bd->si->sensing_conf_data.res_x-1, 0, 0);
    input_set_abs_params(bd->input, ABS_MT_POSITION_Y, 0, bd->si->sensing_conf_data.res_y-1, 0, 0);
    input_set_abs_params(bd->input, ABS_MT_WIDTH_MAJOR, 0, 255, 0, 0);
    input_set_abs_params(bd->input, ABS_MT_TOUCH_MAJOR, 0, 255, 0, 0);
    input_set_abs_params(bd->input, ABS_MT_TRACKING_ID, 0, 255, 0, 0);
	input_set_abs_params(bd->input, ABS_MT_PRESSURE, 0, 255, 0, 0);
