...
//------------------------------------------------------------------------------------
	if(status)
		...
		printk("ft get_value:data_no=%d",data_no);
		...
		if(data_no > 0)//get stress
			...
			printk("ft get_value:value1=%d",gNdtData->value1);
...
static int ndt_stress_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(ndt_i2c_test())
		...
		ndt_stress_power_switch(false);//  tp can't init when stress sensor firmware is bad and i2c is noisy. we must pown down
